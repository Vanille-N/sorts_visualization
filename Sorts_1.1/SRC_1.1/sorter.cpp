...
void Sorter::merge (int a, int b, int c, item * tab []) {
    ...
    while ((i < b) && (i < m_size)) { tab[a + k++] = m_link->send(i++) ; }
    for (int h = a; h < j; h++) {
...
void Sorter::RadixLSD (int baselg) {
    ...
    for (int filter = M-1, d = 0; (filter <= maxval->val) || (filter & maxval->val); filter <<= baselg, d += baselg) {
        ...
        for (int n = 0; n < m_size; n++) { cpy[n] = m_link->send(n) ; }
        for (int n = 0; n < m_size; n++) { access[(cpy[n]->val&filter)>>d]++ ; }
...
void Sorter::RadixMSD (int start, int end, int baselg, int M, int filter, int shift, int level) {
    ...
    item * cpy [end-start] ;
    for (int m = 0; m < M; m++) { access[m] = 0 ; }
    for (int n = start; n < end; n++) { cpy[n-start] = m_link->send(n) ; }
    for (int n = start; n < end; n++) { access[(cpy[n-start]->val&filter)>>shift]++ ; }
    for (int m = 1; m < M; m++) { access[m] += access[m-1] ; }
...
void Sorter::Sampler () {
    ...
    for (int i = sampleSize; i < m_size; i++) { access[Dichotomy(m_link->send(i), 0, sampleSize+1)]++ ; }
    // Turn into cumulative
    for (int j = 1; j <= sampleSize; j++) { access[j] += access[j-1] ; }
    // Copy in order then rewrite
    item * cpy [m_size - sampleSize] ;
    for (int i = m_size-1; i >= sampleSize; i--) { cpy[--access[Dichotomy(m_link->send(i), 0, sampleSize+1)]] = m_link->send(i) ; }
    for (int i = sampleSize; i < m_size; i++) { m_link->receive(i, cpy[i - sampleSize]) ; }
    // Sub-sort
...
void Sorter::Bucket () {
    ...
    // Count members of each interval
    for (int i = 0; i < m_size; i++) { access[m_link->get(i).val / bucketcount]++ ; }
    // Turn into cumulative
    for (int j = 1; j <= bucketcount; j++) { access[j] += access[j-1] ; }
    // Copy in order then rewrite
    item * cpy [m_size] ;
    for (int i = m_size-1; i >= 0; i--) { cpy[--access[m_link->get(i).val / bucketcount]] = m_link->send(i) ; }
    for (int i = 0; i < m_size; i++) { m_link->receive(i, cpy[i]) ; }
    // Sub-sort
